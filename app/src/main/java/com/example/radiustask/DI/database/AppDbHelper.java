...
import static com.example.radiustask.DI.database.RealmUtils.asLiveData;
...
//    }
    ...
    @Override
    public void addAllFacilities(final List<Facility> facilities) {
        mRealm.executeTransaction(new Realm.Transaction() {
            @Override
            public void execute(Realm realm) {
                realm.insertOrUpdate(facilities);
            }
        });
    }

    @Override
    public void addAllExclusionCombinations(final List<ExclusionCombinations> exclusionCombinations) {
        mRealm.executeTransaction(new Realm.Transaction() {
            @Override
            public void execute(Realm realm) {
                realm.insertOrUpdate(exclusionCombinations);
            }
        });
    }

    @Override
    public LiveRealmData<Facility> getAllFacilities() {
        RealmResults<Facility> realmResults = mRealm.where(Facility.class).findAllAsync();
        return asLiveData(realmResults);
    }

    @Override
    public  LiveRealmData<ExclusionCombinations> getAllExclusionCombinations() {
        return asLiveData(mRealm.where(ExclusionCombinations.class).findAllAsync());
    }
    ...
    @Override
    public void deleteAllExclusionCombinations() {
        mRealm.delete(ExclusionCombinations.class);
    }
